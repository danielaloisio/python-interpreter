...
std::vector<Token> Lexer::tokenize() {
    ...
    while (position < code.length()) {
        ...
        switch (current) {
            case '+': tokens.push_back(Token(TokenType::PLUS, "+", line));
                position++;
                break;
            case '-': tokens.push_back(Token(TokenType::MINUS, "-", line));
                position++;
                break;
            case '*': tokens.push_back(Token(TokenType::MULTIPLY, "*", line));
                position++;
                break;
            case '/': tokens.push_back(Token(TokenType::DIVIDE, "/", line));
                position++;
                break;
            case '(': tokens.push_back(Token(TokenType::LPAREN, "(", line));
                position++;
                break;
            case ')': tokens.push_back(Token(TokenType::RPAREN, ")", line));
                position++;
                break;
            case ',': tokens.push_back(Token(TokenType::COMMA, ",", line));
                position++;
                break;
            case ':': tokens.push_back(Token(TokenType::COLON, ":", line));
                position++;
                break;
            case '=':
                ...
                break;
            case '>': tokens.push_back(Token(TokenType::GREATER, ">", line));
                position++;
                break;
            case '<': tokens.push_back(Token(TokenType::LESS, "<", line));
                position++;
                break;
            default:
                throw std::runtime_error(
                    "Unexpected character: " + std::string(1, current) + " one the line " + std::to_string(line));
        }
    }
